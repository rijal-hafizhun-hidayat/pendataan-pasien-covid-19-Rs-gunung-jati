...
$data_edit = mysqli_query($konek, "SELECT * FROM bayar WHERE id_bayar = $id_bayar ");
$tampil = mysqli_fetch_array($data_edit);
if (isset($_POST['edit'])) {
    $nama = $_POST['nama'];
    $gender = $_POST['jenis_kelamin'];
    $level = $_POST['level_ruang'];
    $jumlah = $_POST['jumlah_bayar'];
    $update = mysqli_query($konek, "UPDATE bayar SET nama_pembayar = '$nama', gender = '$gender', level_ruang = '$level', jumlah_bayar = '$jumlah' WHERE id_bayar = '$id_bayar' ");
    if ($update) { ?>
        <script>
                ...
                <div class="container-fluid">
                    ...
                    <form class="bayar" method="POST" action="">
                        <div class="form-group">
                            <div class="form-group">
                                <input type="text" class="form-control" id="exampleInputEmail" value="<?php echo $tampil['nama_pembayar']; ?>" placeholder="Masukkan Nama penghuni" name="nama">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <select class="form-control" aria-label="Default select example" name="level_ruang">
                                    <?php
                                    $nilai = $tampil['level_ruang'];
                                    if ($nilai >= 50000 && $nilai < 1000000) { ?>
                                        <option selected value="500000">Reguler/500.000</option>
                                        <option value="1000000">Premium/1.000.000</option>
                                    <?php } else if ($nilai >= 1000000) { ?>
                                        <option value="500000">Reguler/500.000</option>
                                        <option selected value="1000000">Premium/1.000.000</option>
                                    <?php } ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <input type="text" class="form-control" id="exampleInputPassword" value="<?php echo $tampil['jumlah_bayar']; ?>" placeholder="Masukkan Nominal" name="jumlah_bayar">
                            </div>
                            <div class="form-group">
